...
sub get_data {
    ...
    my $extended_title_offset =
      $self->get_header_size() + 16 +
    ...
    $res .= pack( "a*NNNNN",
        ...
        $self->get_type(), $self->get_codepage(), $self->get_unique_id(),
        $self->get_version() );
    ...
    $res .= pack( "NNNN",
        $vie1, $extended_title_offset, $extended_title_length, $langnumber );
    $res .= pack( "NNNN",
        0xFFFFFFFF, 0xFFFFFFFF, $vie2, $self->get_image_record_index() );
...
sub get_extended_title {
    ...
    my ($title) =
      unpack( "a$exttitlelength", substr( $h, $exttitleoffset - 16 ) );
    return $title;
}
...
sub fix_pointers {
    ...
    if ( $ehlen > 0x98 ) {    #pdurrant
        ...
        if (    $drmoffset != 0xFFFFFFFF
            and $drmoffset > $startblock
            and $offset > 0 )
        {
